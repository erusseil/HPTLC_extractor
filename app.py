...
# Bigger sidebar nav text — the default is quite small for a lab tool
# meant to be comfortable for non-technical users.
st.markdown(
    """
    <style>
...
pg = st.navigation([
    st.Page(home, title="Dashboard", icon="🧪", default=True),
    st.Page("pages/2_📷_Spectractor.py", title="Spectractor", icon="📷"),
    st.Page("pages/3_📈_Visualiser.py", title="Visualiser", icon="📈"),
    st.Page("pages/4_🕸️_Distances.py", title="Distances", icon="🕸️"),
])
pg.run()
